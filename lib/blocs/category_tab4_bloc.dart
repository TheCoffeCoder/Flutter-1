...
class CategoryTab4Bloc extends ChangeNotifier {
  ...
  Future fetchData(int categoryId, mounted) async {
    var response = WpConfig.blockedCategoryIds.isEmpty
      ? await http.get(Uri.parse("${WpConfig.websiteUrl}/wp-json/wp/v2/posts?_embed&categories[]=" + categoryId.toString() + "&page=$_page&per_page=$_postAmountPerLoad"))
      : await http.get(Uri.parse("${WpConfig.websiteUrl}/wp-json/wp/v2/posts?_embed&categories[]=" + categoryId.toString() + "&page=$_page&per_page=$_postAmountPerLoad&categories_exclude=" + WpConfig.blockedCategoryIds));
