...
class WordPressService {
  Future fetchPostsByCategoryIdExceptPostId(
    ...
    var response = await http.get(Uri.parse(
        "${WpConfig.websiteUrl}/wp-json/wp/v2/posts?_embed&exclude=$postId&categories[]=$catId&per_page=$contentAmount"));
  ...
  Future fetchPostsBySearch(String searchText) async {
    var response = WpConfig.blockedCategoryIds.isEmpty
        ? await http.get(Uri.parse(
            "${WpConfig.websiteUrl}/wp-json/wp/v2/posts?_embed&per_page=30&search=$searchText"))
        : await http.get(Uri.parse(
            "${WpConfig.websiteUrl}/wp-json/wp/v2/posts?_embed&per_page=30&search=$searchText&categories_exclude=" +
